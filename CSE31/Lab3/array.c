...
for (i=0;i<n-1;i++){
    for(j=0;j<n-1;j++){
    if((*(s_arr+j)) > (*(s_arr+j+1))){
    //*(s_arr+j)=s_arr[j]& *(s_arr+j+1)=s_arr[j+1]
    temp = *(s_arr+j+1);
...
int bSearch(int *arr, int a, int b, int key){
    // Binary search function. arr is the array, key is the value to search for, 
    // a and b are the boundaries of arr to be searched within.
