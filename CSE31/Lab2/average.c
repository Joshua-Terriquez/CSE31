...
int read_values(double *sum) {   //turned sum into pointer
  int values = 0, input = 0;
  *sum = 0;   //made sum into a pointer
  printf("Enter input values (enter 0 to finish):\n");
  scanf("%d", &input);
  while(input != 0) {
    values++;
    *sum += input; //made sum into pointer
    scanf("%d", &input); //Inputed & due to it missing
  }
  return values;
}
...
int main() {
  double sum = 0;
  int values;
  values = read_values(&sum); /// fixed code by putting &
